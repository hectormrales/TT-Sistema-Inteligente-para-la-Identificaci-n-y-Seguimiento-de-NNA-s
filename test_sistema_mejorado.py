...
3. ✅ TF-IDF ajustado (min_df=1, stopwords español, sin strip_accents)
4. ✅ DBSCAN más permisivo (eps=0.6, min_samples=2)
...
def test_complete_pipeline():
    ...
    print_separator("🚀 SISTEMA MEJORADO: DETECCIÓN DE FEMINICIDIOS CON NNA")
    print("\n📋 Correcciones implementadas:")
    print("   1. ✅ Fuentes RSS especializadas (CIMAC, SEM México, etc.)")
    print("   2. ✅ Detector de feminicidios con patrones específicos")
    print("   3. ✅ TF-IDF ajustado (min_df=1, stopwords español)")
    print("   4. ✅ DBSCAN más permisivo (eps=0.6, min_samples=2)")
    ...
    try:
        df_final = analyzer.run_full_pipeline(
            num_topics=6,
...
def print_statistics(df):
    ...
    if 'tiene_nna' in df.columns:
        ...
        print(f"👶 Con mención de NNA: {with_nna} ({with_nna/total*100:.1f}%)")
    ...
    if 'es_objetivo' in df.columns:
        ...
        if target / total >= 0.50:
            print("   ✅ EXCELENTE: Más del 50% son noticias objetivo")
        elif target / total >= 0.30:
            print("   ✅ ACEPTABLE: Más del 30% son noticias objetivo")
        elif target / total >= 0.10:
            print("   ⚠️  BAJO: Menos del 30% son noticias objetivo")
        else:
            print("   ❌ CRÍTICO: Menos del 10% son noticias objetivo")
            print("      → Considera agregar más fuentes especializadas")
    ...
    if 'cluster' in df.columns:
        ...
        print(f"   Outliers (casos atípicos): {outliers}")
...
def show_target_news_examples(df):
    ...
    if 'prioridad' not in df.columns:
        print("\n⚠️  No hay información de prioridad disponible")
        return
    ...
    if len(high_priority) == 0:
        ...
        print("   - Las fuentes RSS aún no tienen suficiente cobertura de feminicidios")
        print("   - Se necesita agregar más fuentes especializadas")
        
        # Mostrar ejemplos de prioridad MEDIA como alternativa
        medium_priority = df[df['prioridad'] == 'MEDIA'].head(3)
        if len(medium_priority) > 0:
            print("\n📰 Ejemplos de PRIORIDAD MEDIA (segunda mejor opción):")
            for idx, row in medium_priority.iterrows():
                print(f"\n   Noticia #{idx + 1}:")
                print(f"   Título: {row.get('titulo', 'N/A')[:80]}")
                ...
                print(f"   Huérfanos: {'✅' if row.get('tiene_huerfanos', False) else '❌'}")
    else:
...
def show_cluster_analysis(df):
    ...
    print_separator("🔍 ANÁLISIS DE CLUSTERS")
    
    if 'cluster' not in df.columns:
        print("\n⚠️  No hay información de clusters disponible")
        return
    
    clusters = df[df['cluster'] != -1]['cluster'].unique()
    
    if len(clusters) == 0:
        print("\n⚠️  No se formaron clusters (todas las noticias son outliers)")
        print("   Esto puede indicar:")
        print("   - Noticias muy diversas sin temas comunes")
        print("   - Se necesitan más noticias sobre temas similares")
        return
...
def compare_with_previous():
    ...
    print_separator("📊 COMPARACIÓN CON SISTEMA ANTERIOR")
    
    old_file = 'data/noticias_analyzed_with_new_detector.csv'
    new_file = 'data/noticias_sistema_mejorado.csv'
    
    if not os.path.exists(old_file):
        print("\n⚠️  No se encontró archivo de comparación anterior")
        return
    
    if not os.path.exists(new_file):
        print("\n⚠️  Ejecuta primero el pipeline para generar datos nuevos")
        return
    ...
    if 'es_objetivo' in df_new.columns:
        ...
        if 'es_objetivo' in df_old.columns:
            ...
            if improvement > 0:
                ...
                print(f"\n⚠️  Disminución: {improvement} noticias objetivo")
            else:
                print(f"\n➡️  Sin cambio en cantidad")
