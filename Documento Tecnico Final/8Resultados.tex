...
\begin{table}[htbp]
\centering
\caption{Métricas de evaluación del clasificador escalonado sobre el
...
\begin{table}[htbp]
\centering
\caption{Matriz de confusión del clasificador escalonado
(conjunto de prueba)}
...
\begin{tikzpicture}
\begin{axis}[
    title={\textbf{heatmap (n = 41)}},
    xlabel={Predicción},
    ylabel={Etiqueta real},
    xtick={0,1},
    ytick={0,1},
    xticklabels={No relevante, Alta relevancia},
    yticklabels={No relevante, Alta relevancia},
...
\node[white,  font=\Large\bfseries] at (axis cs:0,1) {14};
\node[white,  font=\Large\bfseries] at (axis cs:1,0) {20};
\node[black,  font=\Large\bfseries] at (axis cs:1,1) {6};
\node[black,  font=\Large\bfseries] at (axis cs:0,0) {1};
\end{axis}
\end{tikzpicture}
\caption{Heatmap de la matriz de confusión del clasificador escalonado
sobre el conjunto de prueba ciego ($n=41$). Los valores de la diagonal
principal (VN=14, VP=20) concentran el 83\% de las predicciones.
El único falso negativo (FN=1) corresponde a una noticia con redacción
ambigua.}
...
ubicacion, victimas, ninos\_afectados, edades,situacion\_actual, medios\_contacto y resumen,
con el campo medios\_contacto(posibles contactos de ayuda o institucioes que atienden el caso)
