...
En el ámbito de la inteligencia de fuentes abiertas (OSINT) y el monitoreo mediático comercial,
...
de agregación RSS y rastreadores web de propósito general \cite{weisser2020media}.

No obstante, la debilidad fundamental de estos sistemas radica en su dependencia de cadenas de texto
y expresiones regulares (RegEx). Al carecer de capacidades de inferencia semántica, la búsqueda de
...
validada durante el TT1: el Prototipo~1, basado en 15 palabras clave aisladas con vectorización TF-IDF
y agrupamiento K-Means, registró una tasa de falsos positivos del \textbf{80\%}. El Prototipo~2, que
enriqueció el detector a 72 patrones regex distribuidos en tres categorías (40 de feminicidio, 15 de NNA
y 17 de exclusión), redujo los falsos positivos a un \textbf{60\%}, evidenciando el techo estructural del
paradigma léxico. Estos sistemas son incapaces de discernir sintácticamente entre un reporte donde el
agresor resultó ser menor de edad, una noticia puramente estadística o un caso donde un menor quedó en
orfandad como víctima indirecta, pues la co-ocurrencia de tokens no implica la co-ocurrencia de los
roles semánticos que esos tokens desempeñan en la oración. Esta incapacidad de inferencia contextual
hace que las herramientas tradicionales resulten operativamente ineficaces para el dominio específico
de este proyecto.
...
la sociología forense ha ganado tracción empírica en los ultimos años. La literatura documenta la
transición de modelos de aprendizaje automático superficial (como \textit{Support Vector Machines}
o \textit{Random Forests}) hacia arquitecturas de aprendizaje profundo (Deep Learning) para la
clasificación de discursos de odio, misoginia en redes sociales y categorización automática de reportes
policiales por violencia doméstica \cite{bada2021nlp, garcia2022hate}.
...
\cite{mendoza2023transformers}. Sin embargo al realizar una revisión bibliográfica exhaustiva se
nota una carencia de investigación computacional enfocada en las víctimas indirectas. La inmensa
mayoría de los clasificadores binarios se detienen en la categorización del evento fáctico (feminicidio
vs. homicidio culposo) lo que omite por completo la extracción de entidades relacionadas con los NNA
afectados.
...
manuales exhaustivas en portales de noticias \cite{futuro2023reporte}. Este proceso es no determinista,
...
El análisis del estado actual del arte revela una brecha tecnológica interesante bajo los paradigmas
vigentes que es \textbf{la desconexión entre la recolección masiva de datos no estructurados y la
...
\begin{enumerate}
    \item \textbf{Recolección:} Ingesta concurrente y evasiva de fuentes RSS y motores de búsqueda
          mediante \texttt{StealthSession} v7.0 con emulación de fingerprint TLS.
    \item \textbf{Deduplicación Cross-Site:} Colapso de redundancia mediática mediante una cascada
          de cuatro algoritmos de complejidad creciente (MD5, Jaccard, SimHash y similitud coseno
          TF-IDF), que eliminó el 25.1\% de duplicados en el corpus de validación.
    \item \textbf{Agrupación Temática:} Modelado latente no supervisado mediante reducción
          topológica de dimensionalidad (UMAP) y clústering de densidad variable (HDBSCAN)
          encapsulados en el pipeline BERTopic.
    \item \textbf{Clasificación Escalonada:} Arquitectura neuro-simbólica v7.1 con tres capas
          (Escudo Léxico, Escudo Suave e inferencia BETO Fine-Tuned) que operan sobre un score
          acumulativo con penalizaciones y bonificaciones aditivas.
    \item \textbf{Investigación Profunda Bajo Demanda:} Módulo \texttt{DeepInvestigator} que
          sintetiza fichas de caso estructuradas a partir de búsqueda web (DuckDuckGo) e
          inferencia de lenguaje natural (Gemini Flash), activado manualmente por el analista.
\end{enumerate}

En suma, no existe en el ecosistema civil mexicano una herramienta de software que ejecute este ciclo
completo ---recolección evasiva, deduplicación en cascada, clasificación escalonada neuro-simbólica
e investigación profunda bajo demanda--- de forma unificada y específicamente calibrada para el
problema social de los NNA víctimas indirectas de feminicidio. Este proyecto trasciende la
estadística descriptiva para instaurar una plataforma de inteligencia operativa y de respuesta rápida.
...
defensa perimetral cuyo objetivo primario es la protección contra el scraping
...
de recursos computacionales. En la literatura de recuperación de información y
OSINT se identifican tres vertientes metodológicas principales, que se evaluaron
