...
\begin{NFRequerimientos}
    ...
    \NFRitem{RNF-01}{Optimización de memoria (OOM)}{El modelo BETO no debe provocar cierres abruptos del proceso por saturación de RAM
            durante la inferencia en lote.}{Implementar liberación explícita del caché de PyTorch y reducción dinámica del
    ...
    \NFRitem{RNF-02}{Evasión de contramedidas web (WAF)}
            ...
            sistemáticamente.}
            {Uso del módulo \texttt{StealthSession} con emulación de fingerprint TLS via
            \texttt{cloudscraper}, delays con distribución log-normal y patrón \textit{Circuit Breaker}
            por dominio que evita el bloqueo permanente de IP tras 3 fallos consecutivos.}
            {CU-01, CU-02}
    ...
    \NFRitem{RNF-05}{Procesamiento asíncrono no bloqueante}
            {Las operaciones de larga duración (flujos de recolección y análisis)
            no deben bloquear la interfaz web del Dashboard ni la atención de otras
            peticiones HTTP.}
            {Uso de hilos en segundo plano gestionados con \texttt{threading.Thread}
            y un mapa de estado en memoria consultado por el frontend mediante
            \textit{polling} periódico a los endpoints \texttt{/api/analyze/status}
            y \texttt{/api/investigate/status/<id>}.}
            {CU-01, CU-03}
    ...
    \NFRitem{RNF-06}{Autenticación y control de acceso}
            ...
            \texttt{/register}. Las contraseñas se almacenan como hashes \texttt{bcrypt}
            y nunca en texto plano. La protección contra redirecciones abiertas
            (\textit{Open Redirect}) se valida en el helper
    ...
    \NFRitem{RNF-08}{Trazabilidad por lote de análisis}
            {El sistema debe permitir consultar el historial de ejecuciones
            independientes del flujo y filtrar los datos del Dashboard por
            lote específico para análisis comparativo.}
            {Cada ejecución del flujo asigna un \texttt{batch\_id} único a
